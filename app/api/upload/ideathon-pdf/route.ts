...
async function ensureIdeathonFileBucket() {
  if (!hasServiceRoleAccess()) {
    return;
  }

  const adminClient = createAdminClient();
  const { data: bucket, error: getBucketError } = await adminClient.storage.getBucket(IDEATHON_FILE_BUCKET);
  ...
  if (!bucket) {
    const { error: createBucketError } = await adminClient.storage.createBucket(IDEATHON_FILE_BUCKET, {
      public: true,
  ...
  if (!bucket.public) {
    const { error: updateBucketError } = await adminClient.storage.updateBucket(IDEATHON_FILE_BUCKET, {
      public: true,
      fileSizeLimit: MAX_FILE_SIZE_BYTES,
      allowedMimeTypes,
    });
...
export async function POST(request: Request) {
  try {
    ...
    if (!profile || !["learner", "alumni", "preneur"].includes(profile.role || "")) {
      return NextResponse.json(
        { success: false, error: "파일을 업로드할 권한이 없습니다. SPEC 멤버 계정으로 로그인해주세요." },
        { status: 403 },
      );
    }
    ...
    if (extension !== "pdf" || file.type.toLowerCase() !== "application/pdf") {
      return NextResponse.json(
        {
          success: false,
          error: "지원하지 않는 파일 형식입니다. PDF 파일만 업로드할 수 있어요.",
        },
        { status: 400 },
      );
    }
    ...
    await ensureIdeathonFileBucket();
    ...
    const { error: uploadError } = await storageClient.storage.from(IDEATHON_FILE_BUCKET).upload(filePath, file, {
      cacheControl: "3600",
      upsert: false,
    ...
    const { data: publicUrlData } = storageClient.storage.from(IDEATHON_FILE_BUCKET).getPublicUrl(filePath);
